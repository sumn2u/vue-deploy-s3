...
var data = fs.readFileSync(path.join(__dirname, './../../blueprints/vue-deploy-s3/files/deploy.js'), 'utf-8');

fs.writeFile('deploy.js', data, function (err) {
  if (err) return console.log(err);
  // console.log('Hello World > helloworld.txt');
});
// fs.writeFileSync('deploy.js', data);
// fs.copyFileSync(path.join(__dirname, './../../blueprints/vue-deploy-s3/files/deploy.js'), path.join(__dirname,'/deploy.js'));
log();
